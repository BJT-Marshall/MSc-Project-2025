...
dataset_log_amplitudes -= jnp.mean(dataset_log_amplitudes) #Scale target dataset_log_amplitudes to be centered around zero
dataset_log_amplitudes = dataset_log_amplitudes/jnp.std(dataset_log_amplitudes) #Constrain the variance of the target data set to be 1
...
def lossfun(epsilon, indices):
    inds = jnp.atleast_1d(indices) #Another reformatting step to make sure the indices are in the correct format.
...
def optax_adam_gradient_decent(iterations: int):
    ...
    for i in range(iterations):
        m_sq_error_full = lossfun(epsilon, jnp.arange(len(dataset_log_amplitudes))) #jnp.arange(len(dataset_log_amplitudes)) is just [0,1,2,...,251]        m_sq_error_full_list.append(m_sq_error_full)
...
def yannic_linear_sweeping(iterations: int):
    ...
    epsilon = vs.parameters["epsilon"]  # revert epsilon tensor in case previous optimisation scheme has been run
    ...
    for i in range(iterations):
        for reference_site in np.arange(L):
            ...
            m_sq_error_full = lossfun(learning.epsilon, jnp.arange(len(dataset_log_amplitudes))) #loss function of the learning objects current epsilon tensor against the exact state 
...
def test_iterations():
    ...
    for element in list:
        ...
        test_amp, test_error = lasso_linear_sweeping(iterations=element, indices=jnp.arange(len(dataset_log_amplitudes)))
        print("LASSO: ", np.array(test_error)[0])
    return None

#----------------------------------------------------------------------------------------------------------------------------------------------
def lasso_linear_sweeping(iterations: int, indices: list):
    ...
    learning = QGPSLogSpaceFit(epsilon) #The way of interfacing the learning model with the qGPS state

    #Define LASSO Model
    model = Lasso(alpha=0) #alpha is the 'lambda' parameter, defines the L1 penalty the model uses. REGULARISATION
    model.n_iter_=iterations
    m_sq_error_full_list = []

    for site in np.arange(L): #For each single site perform an optimisation cycle
        learning.ref_sites = site
    
        K = learning.set_kernel_mat(update_K = True, confs = dataset_configs[indices]) #Setting the kernel function (feature vector) excluding the current reference site using the sampled amplitudes 
        ...
        optimal_weights = model.fit(X = K, y=dataset_log_amplitudes[indices]).coef_
...
#test_amp, test_error = lasso_linear_sweeping(iterations=100, indices=indices)

#Test outputting the exact data and the learnt data
#print(dataset_log_amplitudes[indices])
#print(test_error)
