...
def lasso_linear_sweeping(tol: float, iterations: int, indices: list, configs: list, amps:list, log_amps: list, alpha: float, vs, ha, weighted_according_to_psi_squared, tempSeed):
...
for m_i in M_list:
  ...
  for i in [1,11]:
    ...
    norm_amps = jnp.exp(log_amps)/jnp.sqrt(jnp.sum(abs(jnp.exp(log_amps))**2))
    overlaps.append(float(abs(estimated_amps.T.dot(norm_amps))))
